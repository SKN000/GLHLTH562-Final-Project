...
makeCodebook(data = password_data, output_dir = getwd(), report_title = "Data Codebook", open_report = FALSE, replace = TRUE)
...
## Study and show common characteristics(category, length, characters, etc.) of popular passwords.
...
```{r}
library(dplyr)
category_distribution <- password_data %>%
  group_by(category) %>%
  summarise(count = n())

ggplot(category_distribution, aes(x=reorder(category, -count), y=count, fill=category)) +
  geom_bar(stat="identity") +
  coord_flip() +
  labs(title="Password Category Distribution", x="Category", y="Frequency") +
  theme_minimal() +
...
| Tweet  | <https://twitter.com/AdityaDahiyaIAS/status/1757779392137240651>                                  |
| Code   | <https://github.com/Aditya-Dahiya/projects_presentations/blob/main/projects/tidy_valentine_day.R> |
...
1.  Learned a new fancy plot "geom_chicket".

2.  Learned how to import and use some fancy font, which is so important for some topics like this one.

3.  Learned how to customize almost all the elements in the plot in theme().
